   ...
   return (
      <div className="container max-w-screen-lg mx-auto p-2 md:mt-12">
         <div className="grid grid-cols-1 md:grid-cols-3 gap-5 text-center ">
            {cartFeatures.map((cart) => {
               return <Feature key={cart.id} cart={cart} />;
            })}
         </div>
      </div>
   );
...
const Feature = ({ cart }) => {
   return (
      <div className={`p-6 border-red-100 shadow-md  rounded-2xl`}>
         <div className="flex text-red-300 justify-center text-5xl mb-1 ">
            {cart.icon}
         </div>
         <h2 className="mb-4 text-red-100  font-semibold text-xl ">
            {cart.title}
         </h2>
